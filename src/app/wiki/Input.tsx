...
export default function Input() {
    ...
    return (
        <div className="flex flex-col gap-2 w-60 min-[460px]:w-70 sm:w-80">
            <div className="flex items-center bg-[#1D1D1D] border border-[#303030] rounded-xl h-10 px-2.5">
                ...
                <input value={value} onChange={e => setValue(e.target.value)} type="text" placeholder="Пошук за ключовим словом" className="bg-transparent border-none outline-none text-[16px] placeholder:text-[#C3C3C3] font-light w-full" />
            </div>
            {value.length > 0 && <div className="bg-[#1D1D1D] rounded-xl absolute w-80 box-border mt-13 border border-[#303030] max-h-80 overflow-x-scroll">
